...
func string_to_id(string: String) -> String:
	var special_chars := RegEx.create_from_string("[\\W_]+")
	return special_chars.sub(string.to_lower(), " ", true).strip_edges().replace(" ", "_")
...
func string_to_id_unique(string: String, existing_ids: Array[String]) -> String:
	...
	while existing_ids.has(id):
		var num: int = int(digit.search(id).get_string())
		id = digit.sub(id, str(num+1))
	return id
